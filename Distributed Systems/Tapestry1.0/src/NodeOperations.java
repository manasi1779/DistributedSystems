	...
	public void addFile(String[] optionArr) {
		...
		try {
			...
			if (fileToFind.exists() && fileToFind.isFile()) {
				...
				FileInputStream fisToClient = new FileInputStream(fileToFind);
				BufferedInputStream bisClient = new BufferedInputStream(fisToClient);
		...
		try {
			...
			for (int i = 0; i < noOfFiles; i++) {
				try {
					...
					File x = new File(homedir.getAbsolutePath() + "/" + InetAddress.getLocalHost().getHostName() + "/"
							+ fileName);
