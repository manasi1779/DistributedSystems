...
public class RaymondAlgorithm implements Runnable{
	...
	static String peerNames[] = new String[3];
	...
	/**
	 * Initializes sockets and threads
	 */
	public void init(){
		try {
			...
			if(myIndex>0){
				...
				peerNames[0] = addressTable.get((myIndex-1)/2);
			...
			if(myIndex<3){
				for(int i = 1; i < 3; i++){
					...
					peerNames[i] = addressTable.get(myIndex*2+i);
	...
	/**
	 * Requests holder for shared data
	 */
	...
	public void tokenReceived(){	
		if(!queue.isEmpty()){
			...
			if(requestor < 0){
				...
				System.out.println("Sent token to "+peerNames[requestor]);
				holder = requestor;
				if(!queue.isEmpty()){
					writeToChannel(holder, "DATA");
					System.out.println("Sent request to "+peerNames[holder]);
				}
			}
		}
		else{
	...
	public void requestReceived(int channelNo){
		...
		if(holder<0 && !usingData){
			synchronized(eventObject){
				...
				System.out.println("Sent token to "+peerNames[queue.get(0)]);
				queue.remove(0);
			}				
			holder = channelNo;
		}
	...
	private void readChannel(int channelNo) {
		...
		switch(response){
			...
			case "DATA":{
				System.out.println("Request received "+peerNames[channelNo]);
